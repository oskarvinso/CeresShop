...
const Navbar: React.FC<NavbarProps> = ({ user, activeView, setView }) => {
  const navItems: { label: string; view: AppView; icon: React.ReactNode }[] = [
    ...
    { label: 'Publicar', view: 'sell', icon: <PlusCircle size={18} /> },
  ];
    ...
    <header className="sticky top-0 z-50 bg-white/80 backdrop-blur-md border-b border-pink-100">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-16">
          ...
          >
            <div className="mr-3 overflow-hidden rounded-lg w-10 h-10 border border-gray-100 group-hover:scale-110 transition-transform bg-white flex items-center justify-center">
          ...
          <div className="flex items-center space-x-4">
            <button 
              ...
              className="flex items-center space-x-2 bg-pink-50 px-3 py-1.5 rounded-full hover:bg-pink-100 transition-colors"
            >
              <img 
                src={user.avatar} 
                alt={user.name} 
              ...
              />
              <span className="hidden sm:inline font-medium text-gray-700 text-sm">{user.name}</span>
            </button>
          </div>
        </div>
      </div>
